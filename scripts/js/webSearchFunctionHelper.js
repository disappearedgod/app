...
function pageNumberCut(tolNum, partNum){
	...
	var pHtml = '<div data-pid=1 data-pbn='+pblock+'><input class="prePage" type="button" value="'+langNational.prev+'"></input>';
	pHtml += '<p style="display:inline;border:none;margin:8px;">...</p>';
	...
	if(pblock > 1){
		pHtml += '<input class="nextPage" type="button" value="'+langNational.next+'"></input></div>';
	}
	return pHtml;
}
...
function htmlLangChange(langNational){
	...
	$('#useredit div').eq(0).find("span").eq(0).text(langNational['language']);
	$('#useredit div').eq(1).find("span").eq(0).text(langNational['searchnum']);
	$('#useredit div').eq(2).find("span").eq(0).text(langNational['recomnum']);
	$('#useredit div').eq(3).find("span").eq(0).text(langNational['usersure']);
	$('#useredit div').eq(3).find("span").eq(1).text(langNational['usercancel']);
}
